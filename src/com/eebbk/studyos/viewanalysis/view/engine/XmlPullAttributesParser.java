...
import com.eebbk.studyos.viewanalysis.view.utils.XmlUtils;
...
class XmlPullAttributesParser implements XmlResourceParser {
	...
	public int next() throws XmlPullParserException, IOException {
		if (!mStarted) {
			...
			/*
			 * if (ParserFactory.LOG_PARSER) { System.out.println("STRT " +
			 * mParser.toString()); }
			 */
		...
		/*
		 * if (ParserFactory.LOG_PARSER) { System.out.println("NEXT " +
		 * mParser.toString() + " " + eventTypeToString(mEventType) + " -> " +
		 * eventTypeToString(ev)); }
		 * 
		 * if (ev == END_TAG && mParser.getDepth() == 1) { // done with parser
		 * remove it from the context stack. // ensurePopped();
		 * 
		 * if (ParserFactory.LOG_PARSER) { System.out.println(""); } }
		 */
	...
	public String nextText() throws XmlPullParserException, IOException {
		...
		if (eventType == TEXT) {
			...
			if (eventType != END_TAG) {
				throw new XmlPullParserException(
						getPositionDescription() + ": event TEXT it must be immediately followed by END_TAG", this,
						null);
			}
			return result;
