...
public class YDLayoutInflate {
	 ...
	 * (LayoutInflater == null) { throw new AssertionError(
	 * "LayoutInflater not found."); } return LayoutInflater; }
	 */
	...
	public View inflate(int resource, ViewGroup root, boolean attachToRoot) {
		...
		if (DEBUG)
			System.out.println("INFLATING from resource: " + resource);
		XmlPullParser parser = mContext.getResources().getLayout(resource);
	...
	public final View createView(String name, String prefix, AttributeSet attrs)
		...
		try {
			if (constructor == null) {
				...
				System.out.println("prefix + name = " + (prefix + name) + " ,,, name = " + name);
	...
	protected View onCreateView(String name, AttributeSet attrs) throws ClassNotFoundException {
		...
		return createView(name, "com.eebbk.studyos.viewanalysis.view.VA", attrs);
	}
	...
	View createViewFromTag(String name, AttributeSet attrs) {
		...
		try {
			View view = (mFactory == null) ? null : mFactory.onCreateView(name, mContext, attrs);
	...
	private void rInflate(XmlPullParser parser, View parent, final AttributeSet attrs)
		...
		while (((type = parser.next()) != XmlPullParser.END_TAG || parser.getDepth() > depth)
			...
			if (TAG_REQUEST_FOCUS.equals(name)) {
				...
				Log.d("viewhash", view.getId() + "  :addView");
				System.out.println("子布局数量：" + viewGroup.getChildCount());
		...
		if (parent instanceof ViewGroup) {
			...
			if (layout == 0) {
				...
				if (value == null) {
					...
					throw new InflateException("You must specifiy a valid layout " + "reference. The layout ID " + value
							+ " is not valid.");
				}
			} else {
				...
				try {
					...
					if (TAG_MERGE.equals(childName)) {
						 ...
						 * View_id , View.NO_ID); // While we're at it, let's
						 * try to override android:visibility. int visibility =
						 * a.getInt
